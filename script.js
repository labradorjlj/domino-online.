...
// Inicializa a conexão do socket de forma segura quando a página carrega por completo
...
function setupSocketListeners() {
    ...
    // 🐍 RECONSTRUÇÃO DA MESA EM FORMATO SERPENTE ENCADEADA (FOTO 2)
        ...
        if (data.mesa && Array.isArray(data.mesa) && data.mesa.length > 0) {
            ...
            correnteVisual.forEach((pedra) => {
                ...
                if (pedra.ladoA === pedra.ladoB) {
                    ...
                    pieceDiv.innerHTML = `${gerarHtmlFace(pedra.ladoA)}<div class="line-vert"></div>${gerarHtmlFace(pedra.ladoB)}`;
                } else {
                    pieceDiv.className = 'domino-piece piece-horizontal';
                    pieceDiv.innerHTML = `${gerarHtmlFace(pedra.ladoA)}<div class="line-horiz"></div>${gerarHtmlFace(pedra.ladoB)}`;
                }

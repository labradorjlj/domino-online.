...
// GERA O CORPO DA PEDRA COM GRID DE DADO
...
// FIX DE RENDERIZAÇÃO DA MESA ESTILO COMPACTO CONTÍNUO (EVITA ACÚMULO DE TEXTO)
socket.on('atualizarMesa', (data) => {
    ...
    // Alinha as pedras lado a lado de forma flexível e elegante na mesa virtual
    if (data.mesa && Array.isArray(data.mesa)) {
        data.mesa.forEach((pedra) => {
            const pieceDiv = document.createElement('div');
            
            // Se os dois lados forem iguais, renderiza na vertical (Bucha) igual no dominó de verdade
            if (pedra[0] === pedra[1]) {

...
function desenharMao(hand) {
    ...
    hand.forEach(piece => {
        ...
        div.onclick = () => {
            ...
            // Se as pontas forem [-1, -1], significa que a mesa foi resetada (Nova Rodada). 
            // O jogador da vez pode iniciar com QUALQUER pedra.
...
socket.on('atualizarMesa', (data) => {
    ...
    // Atualiza o placar das Duplas no topo da tela em tempo real
    if (data.score) {
    ...
    if (statusMesa) {
        if (socket.id === turnoDeQuem) {
            statusMesa.innerText = "SUA VEZ! Jogue qualquer pedra para abrir a rodada." : "SUA VEZ DE JOGAR! 🫵";
            if (pontasDaMesa[0] === -1) statusMesa.innerText = "SUA VEZ! Abra a rodada com a pedra que quiser! 🃏";

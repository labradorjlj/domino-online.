...
function desenharMao(hand) {
    ...
    hand.forEach(piece => {
        ...
        div.onclick = () => {
            ...
            // Regra: Se for a primeira pedra da rodada (mesa limpa), joga direto sem escolher lado
...
// Função acionada pelo botão de pular vez na tela
function pularVez() {
...
socket.on('atualizarMesa', (data) => {
    ...
    if (statusMesa) {
        if (socket.id === turnoDeQuem) {
            ...
            if (btnPassar) btnPassar.classList.remove('hidden'); // Mostra botão se for sua vez
        } else {
            statusMesa.innerText = "Aguardando o próximo jogador... ⏳";
            statusMesa.style.color = "rgba(255,255,255,0.5)";
            if (btnPassar) btnPassar.classList.add('hidden'); // Esconde se não for sua vez
        }
    }

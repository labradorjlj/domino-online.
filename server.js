...
function iniciarRodadaOficial(room, baralhoGerado, regraEscolhida) {
    ...
    // Envia a mão atualizada para cada jogador humano
    ...
    // Dispara a mesa limpa apontando o turno correto
    ...
    // CORREÇÃO: Força o Robô a iniciar a partida se ele tiver a pedra de saída
...
function rodarInteligenciaBot(room) {
    ...
    // CORREÇÃO: Robô agora identifica corretamente qual pedra iniciar com base na votação ativa
    if (room.mesa.length === 0) {
        let pedraDeSaida = bot.hand[0]; // Padrão de segurança
...
function executarJogadaServidor(room, jogador, pedra, ladoDaMesa) {
    ...
    // Avisa os humanos sobre a mão modificada após jogar
    room.players.forEach(p => {

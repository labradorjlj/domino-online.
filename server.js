...
function processarDuplasPorVotacao(room) {
    ...
    // Confere se houve par mútuo (1 escolheu 2 e 2 escolheu 1)
    for(let i=0; i<4; i++) {
        for(let j=i+1; j<4; j++) {
            if(room.escolhasParceiros[p[i].id] === p[j].id && room.escolhasParceiros[p[j].id] === p[i].id) {
                // Monta a primeira dupla encontrada
                p[i].dupla = 'A';
                p[j].dupla = 'A';
                
                // Os outros dois formam a Dupla B obrigatoriamente
    ...
    // Se todos votaram em pessoas diferentes, monta de forma automática padrão
    if(!duplaFormada) {
        p[0].dupla = 'A'; p[1].dupla = 'B';
        p[2].dupla = 'A'; p[3].dupla = 'B';
    }

    // Envia para a fase de votação de regras
...
function verificarJogoTrancado(room) {
    if (room.passadasSeguidas >= 4) {
        ...
        room.ultimoVencedorId = grandfatherId;
...
io.on('connection', (socket) => {
    socket.on('joinRoom', ({ roomId, playerName }) => {
        ...
        // Quando junta 4 players reais, abre a tela de escolher parceiro

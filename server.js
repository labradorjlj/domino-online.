...
// Função auxiliar para iniciar uma nova rodada mantendo os pontos
function iniciarNovaRodada(room, jogadorInicialId) {
...
io.on('connection', (socket) => {
    ...
    socket.on('jogarPedra', ({ roomId, pedra, ladoDaMesa }) => {
        ...
        // Primeira peça da rodada (Aceita qualquer pedra livremente)
        ...
        // CONDIÇÃO DE VITÓRIA: Jogador Bateu
        if (jogador.hand.length === 0) {
            ...
            room.ultimoVencedorId = socket.id; // Ele começa a próxima rodada com a pedra que quiser

            io.to(roomId).emit('roundEnded', {
                vencedor: vencedorDupla, score: room.score, motivo: `${jogador.name} Bateu!`
            });

            // Começa nova partida automaticamente após 3 segundos
            setTimeout(() => {

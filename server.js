...
// 🛡️ CORREÇÃO CRÍTICA: Garante que o servidor não trave tentando enviar socket para robôs virtuais
function iniciarRodadaOficial(room, baralhoGerado, jogadorInicialId) {
    ...
    // Distribui as mãos individualmente
    room.players.forEach((player, index) => {
        player.hand = baralhoGerado.slice(index * 7, (index + 1) * 7);
        // Só emite se for um jogador humano conectado via socket real
    ...
    // Atualiza o tabuleiro globalmente para todos os humanos na sala
    ...
    // Se o primeiro a jogar for um robô, ativa a inteligência dele
...
io.on('connection', (socket) => {
    ...
    socket.on('adicionarRobos', ({ roomId }) => {
        ...
        while (room.players.length < 4) {
            ...
            // O robô vota direto para agilizar
    ...
    socket.on('castVote', ({ roomId, vote }) => {
        ...
        // Quando todos os 4 votos forem computados (incluindo humanos e robôs)
        if (room.votes.votedCount >= 4) {
            ...
            // Define quem tem a bucha de início

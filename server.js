...
io.on('connection', (socket) => {
    ...
    socket.on('jogarPedra', ({ pedra, lado }) => {
        ...
        const idx = jogador.hand.findIndex(p => (p.ladoA === pedra.ladoA && p.ladoB === pedra.ladoB) || (p.ladoA === pedra.ladoB && p.ladoB === pedra.ladoA));
        if (idx === -1) return socket.emit('errorMsg', 'Pedra não encontrada na sua mão!');

        executarJogadaServidor(room, jogador, jogador.hand[idx], lado);
    });
    ...
    socket.on('castVote', ({ vote }) => {
        ...
        if (vote === '6-6' || vote === '1-1') {
            room.votes[vote] = (room.votes[vote] || 0) + 1;
            room.votes.votedCount = (room.votes.votedCount || 0) + 1;

            if (room.votes.votedCount >= room.players.filter(p => !p.isBot).length) {
                const regraVencedora = room.votes['1-1'] > room.votes['6-6'] ? '1-1' : '6-6';
                iniciarRodadaOficial(room, criarBaralho(), regraVencedora);
            }
        }
    });

...
io.on('connection', (socket) => {
    // Evento para quando o jogador desconecta ou fecha a aba
    socket.on('disconnect', () => {
        Object.keys(rooms).forEach(roomId => {
            const room = rooms[roomId];
            room.players = room.players.filter(p => p.id !== socket.id);
            
            // Se todo mundo saiu da sala, deleta ela da memória
            if (room.players.length === 0) {
                delete rooms[roomId];
            } else {
                // Se ainda tem gente, atualiza o contador dos que ficaram
                io.to(roomId).emit('roomUpdated', {
                    players: room.players.map(p => ({ name: p.name, dupla: p.dupla })),
                    count: room.players.length
                });
                // Se o jogo estava rolando e alguém saiu, volta pro lobby
    ...
    socket.on('joinRoom', ({ roomId, playerName }) => {
        ...
        // SE A SALA JÁ EXISTIR MAS ESTIVER CHEIA E O JOGADOR ESTIVER TENTANDO CRIAR OUTRA
        if (rooms[targetRoomId] && rooms[targetRoomId].players.length >= 4) {
            // Se por algum motivo as sessões antigas travaram e a sala ficou fantasma,
            // vamos resetar se for o caso, senão barramos
    ...
    socket.on('jogarPedra', ({ roomId, pedra, ladoDaMesa }) => {
        ...
        if (player.hand.length === 0) {
            ...
            io.to(roomId).emit('roundEnded', {
                vencedor: vencedorDupla, score: room.score, motivo: `${player.name} Bateu!`
            });

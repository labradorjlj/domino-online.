...
io.on('connection', (socket) => {
    socket.on('disconnect', () => {
        Object.keys(rooms).forEach(roomId => {
            ...
            // 🛡️ CORREÇÃO: Filtra apenas conexões humanas reais para evitar queda falsa devido aos robôs virtuais
            const sobrouHumano = room.players.some(p => p.id === socket.id && !p.isBot);
            if (!sobrouHumano) return;
    ...
    socket.on('castVote', ({ roomId, vote }) => {
        ...
        if (room.votes.votedCount === 4) {
            ...
            let { id: jogadorInicialId } = room.players[0]; 

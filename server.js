...
function forcarJogadaAutomatica(room) {
    ...
    // Procura a primeira pedra que serve
    ...
    // Se não tiver nenhuma, passa automaticamente
...
io.on('connection', (socket) => {
    socket.on('joinRoom', ({ roomId, playerName }) => {
        ...
        if (!rooms[targetRoomId]) {
            rooms[targetRoomId] = {
                ...
                tempoRestante: 40, timerInterval: null
            };
        }
    ...
    socket.on('passarVez', ({ roomId }) => {
        ...
        // TRAVA CRÍTICA: Verifica se tem pedras utilizáveis na mão antes de deixar passar
        ...
        if (room.mesa.length > 0) {
            ...
            temPedraValida = true; // Se a mesa tá vazia ele obrigatoriamente joga
        }

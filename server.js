...
io.on('connection', (socket) => {
    ...
    socket.on('adicionarRobos', ({ roomId }) => {
        ...
        // Se completou com bots, inicia a partida direto sem prender na votação
        iniciarPartidaDireto(room);
    });
    ...
    socket.on('castVote', ({ roomId, vote }) => {
        ...
        if (room.votes.votedCount >= 4) {
            ...
            let grandfatherId = room.players[0].id; 
            let numeroProcurado = regraGanhadora === '6-6' ? 6 : 1;
            ...
            iniciarRodadaOficial(room, baralho, grandfatherId);
        }
    });

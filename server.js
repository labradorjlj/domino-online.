...
io.on('connection', (socket) => {
    ...
    // 🤖 ADICIONAR ROBÔS AGORA ABRE A TELA DE VOTAÇÃO DA REGRA DE SAÍDA OBRIGATORIAMENTE
    socket.on('adicionarRobos', ({ roomId }) => {
        ...
        // Define as duplas automáticas tradicionais cruzadas (1 e 3 vs 2 e 4)
        room.players[0].dupla = 'A'; room.players[2].dupla = 'A';
        room.players[1].dupla = 'B'; room.players[3].dupla = 'B';

        // Altera o estado do jogo e envia para a tela de votação (Sena ou Pio)
        room.gameState = 'voting';
        io.to(room.id).emit('startVoting', { nomesDuplas: obterNomesDuplas(room) });
    });
    ...
    socket.on('castVote', ({ roomId, vote }) => {
        ...
        // Se estiver jogando com bots, o jogador decide sozinho a regra e ela já ganha
        ...
        // Fluxo normal para 4 jogadores reais votando
        room.votes[vote]++;
